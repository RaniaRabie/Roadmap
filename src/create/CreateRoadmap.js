...
const DnDFlow = () => {
  ...
  const handleAddLink = () => {
    setLinks([...links, { type: "", EnOrAr: "", title: "", url: "" }]);
  };
  ...
  // Function to publish nodes to the JSON server
  // const publishNodes = () => {
  //   axios
  //     .post("http://localhost:3200/roadmaps", { dataToPublish })
  //     .then((response) => {
  ...
  const publishNodes = () => {
      ...
      axios
        .post("http://localhost:3200/roadmaps", { data: dataString }) // Send as a string
        .then((response) => {
          console.log("Nodes data successfully stored:", response.data);
        })
        .catch((error) => {
  ...
  const updateNodes = () => {
    ...
    axios
      .put(`http://localhost:3200/roadmaps/${id}`, { dataToPublish })
      .then(() => {
